-- Migration: Fix view to prioritize operation sequence over timestamp
-- Date: 2025-12-22
-- Description: The view should show the highest operation sequence (op004 > op003 > op002 > op001)
--              not just the latest timestamp. This ensures op002 is shown even if op001 happened later.

-- ============================================================================
-- LOGS DATABASE
-- ============================================================================

-- Create helper function to get operation sequence number
CREATE OR REPLACE FUNCTION get_operation_sequence(operation_id TEXT)
RETURNS INTEGER AS $$
BEGIN
    CASE LOWER(operation_id)
        WHEN 'op001' THEN RETURN 1;
        WHEN 'op002' THEN RETURN 2;
        WHEN 'op003' THEN RETURN 3;
        WHEN 'op004' THEN RETURN 4;
        ELSE RETURN 0;
    END CASE;
END;
...
-- Recreate view with operation sequence priority (not timestamp priority)
...
WITH latest_operations AS (
    ...
    -- Prioritize by operation sequence, then by timestamp
    ...
    ORDER BY job_id, 
             ...
             completed_at DESC  -- Then by timestamp
),
latest_scanned_operations AS (
    ...
    ORDER BY 
        ...
        sc.scanned_at DESC  -- Then by timestamp
)
SELECT 
    jo.job_id,
    ...
    -- Prioritize by operation sequence, not timestamp
    -- op004 > op003 > op002 > op001
    COALESCE(
        CASE 
            WHEN get_operation_sequence(ls.operation_id) > get_operation_sequence(lo.latest_completed_operation_id)
            THEN ls.operation_id
            WHEN get_operation_sequence(ls.operation_id) = get_operation_sequence(lo.latest_completed_operation_id)
                 AND ls.latest_completed_at_raw > COALESCE(lo.latest_completed_at_raw, '1970-01-01'::timestamptz)
            THEN ls.operation_id
    ...
    COALESCE(
        CASE 
            WHEN get_operation_sequence(ls.operation_id) > get_operation_sequence(lo.latest_completed_operation_id)
            THEN ls.latest_completed_at_raw AT TIME ZONE 'Australia/Sydney'
            WHEN get_operation_sequence(ls.operation_id) = get_operation_sequence(lo.latest_completed_operation_id)
                 AND ls.latest_completed_at_raw > COALESCE(lo.latest_completed_at_raw, '1970-01-01'::timestamptz)
            THEN ls.latest_completed_at_raw AT TIME ZONE 'Australia/Sydney'
            ELSE lo.latest_completed_at_raw AT TIME ZONE 'Australia/Sydney'
        END,
        lo.latest_completed_at_raw AT TIME ZONE 'Australia/Sydney',
        ls.latest_completed_at_raw AT TIME ZONE 'Australia/Sydney'
...
COMMENT ON VIEW job_status_view IS 'Pre-computed view of job status grouped by job_id. latest_completed_operation_id prioritizes operation sequence (op004 > op003 > op002 > op001) over timestamp. All timestamps are displayed in Australia/Sydney timezone.';

COMMENT ON FUNCTION get_operation_sequence(TEXT) IS 'Returns sequence number for operation: op001=1, op002=2, op003=3, op004=4. Used to prioritize operations by sequence rather than timestamp.';
